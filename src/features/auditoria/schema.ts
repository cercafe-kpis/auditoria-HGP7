 ...
 * todos los campos obligatorios y tiquete no vacío. La fotografía es
 * OPCIONAL (a pedido del usuario) — por eso `fotos` ya no exige un mínimo
 * de elementos. Esta MISMA validación (Zod) es la que decide si el botón
 * de guardar se habilita; no existe un backend adicional que la repita,
 * así que aquí es literalmente la única línea de defensa además de las
 * columnas "obligatorias" configuradas en la Lista de SharePoint.
 */
